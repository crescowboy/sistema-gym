...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        console.log("[Login] Éxito. Cookies:", document.cookie)
        console.log("[Login] Redirigiendo a dashboard...")
        setTimeout(() => router.push("/dashboard"), 500)
      } else {
        setError(data.message || "Las credenciales son incorrectas")
      }
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <CardContent className="space-y-6">
          {error && (
            <div className="p-3 bg-red-500/20 border border-red-500/50 rounded-md text-red-200 text-sm font-sans">
              {error}
            </div>
          )}
